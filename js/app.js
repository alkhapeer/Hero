...
function getMyCourses() {
    return []; // لا نستخدم دوراتي، نعيد مصفوفة فارغة
}

function saveMyCourse(courseId) {
    // لا تفعل شيئاً (دالة وهمية لأن قسم دوراتي محذوف)
    // تُستدعى عند الضغط على زر الدورة في بعض الإصدارات
}

function isMyCourse(courseId) {
    return false; // لا نستخدم دوراتي
}
...
// ================================
// صفحة الدورات (بطاقات مباشرة برابط Google Drive)
// ================================
function renderHome() {
    ...
    if (categoriesData.length) {
        categoriesData.forEach(cat => {
            ...
            html += `<h2 style="border-bottom:2px solid #e5e7eb;padding-bottom:8px;margin-bottom:15px;">${esc(cat.icon || '')} ${esc(cat.title)}</h2>
