...
function renderAcademy() {
    app.innerHTML = `
        <div style="min-height:100vh;background:#f8fafc;padding:25px 15px;">
            <div style="max-width:850px;margin:auto;">
                ...
                <button onclick="location.hash='#courses'" style="width:100%;background:#fff;border:0;border-radius:18px;padding:22px;margin-bottom:15px;text-align:right;cursor:pointer;display:flex;align-items:center;gap:18px;">
                    ...
                    <div style="flex:1;"><div style="font-weight:bold;font-size:19px;">الدورات المتاحة</div><div style="color:#64748b;font-size:14px;">تصفح جميع الدورات</div></div>
                </button>
...
function renderHome() {
    let html = `<div style="max-width:900px;margin:auto;padding:20px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:20px;">
            ...
            <h1 style="font-size:25px;margin:0;">📚 الدورات المتاحة</h1>
        </div>`;
    ...
    if (categoriesData.length) {
        categoriesData.forEach(cat => {
            ...
            html += `<h2 style="border-bottom:2px solid #e5e7eb;padding-bottom:8px;margin-bottom:15px;">${esc(cat.icon || '')} ${esc(cat.title)}</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:15px;">`;
            catCourses.forEach(course => {
                const url = course.drive_url || course.url || '';
                html += `<div style="background:#fff;border:1px solid #e0e0e0;border-radius:12px;padding:20px;display:flex;flex-direction:column;justify-content:space-between;">
                    <div><div style="font-size:36px;">${esc(course.icon)}</div><h3>${esc(course.title)}</h3><p style="color:#666;font-size:14px;">${esc(course.description || '')}</p>
                    <ul style="color:#475569;font-size:13px;padding-left:20px;">${(course.features || []).map(f => `<li>${esc(f)}</li>`).join('')}</ul></div>
        ...
        visibleCourses.forEach(course => {
            const url = course.drive_url || course.url || '';
            html += `<div style="background:#fff;border:1px solid #e0e0e0;border-radius:12px;padding:20px;"><div style="font-size:36px;">${esc(course.icon)}</div><h3>${esc(course.title)}</h3><p>${esc(course.description || '')}</p><a href="${esc(url)}" target="_blank" rel="noopener" style="display:block;text-align:center;background:#2563eb;color:#fff;text-decoration:none;padding:10px;border-radius:8px;font-weight:bold;">📥 تحميل الدورة</a></div>`;
        });
        html += `</div>`;
    }
    html += `</div>`;
